...
public class MainActivity extends AppCompatActivity {
    ...
    private void initializeSearchView(){
        EditText idEditSearch = findViewById(R.id.idEditSearch);
        idEditSearch.setOnEditorActionListener(new EditText.OnEditorActionListener() {
            @Override
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                if (actionId == EditorInfo.IME_ACTION_DONE) {
                    // on below line calling method to search tracks.
                    searchTracks(idEditSearch.getText().toString());
                    return true;
                }
                return false;
            }
    ...
    private String getToken(){
        ...
        return sp.getString("token","Not Found");
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    editor.putString("token","Bearer " + tk);
                    editor.apply();
    ...
    private void initializeAlbumsRV(){
        RecyclerView idRVAlbums = findViewById(R.id.idRVAlbums);

        ArrayList<AlbumModel> albumModelArrayList = new ArrayList<>();
        AlbumAdapter albumAdapter = new AlbumAdapter(albumModelArrayList, this);
        idRVAlbums.setAdapter(albumAdapter);
        ...
        JsonObjectRequest albumObjRequest = new JsonObjectRequest(Request.Method.GET, url, null, new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                try {
                    ...
                    for (int i = 0; i < albumArray.length(); i++) {
                        ...
                        albumModelArrayList.add(new AlbumModel(album_type, artistName, external_ids, external_urls, href, id, imgUrl, label, name, popularity, release_date, total_tracks, type));
                    }
    ...
    private void initializePopularAlbumsRV(){
        ...
        ArrayList<AlbumModel> albumRVModalArrayList = new ArrayList<>();
            ...
            public void onResponse(JSONObject response) {
                try {
                    ...
                    for (int i = 0; i < albumArray.length(); i++) {
                        ...
                        String type = albumObj.getString("type");
                        // on below line adding data to array list.
                        albumRVModalArrayList.add(new AlbumModel(album_type, artistName, external_ids, external_urls, href, id, imgUrl, label, name, popularity, release_date, total_tracks, type));
                    }
